...
- **One step, then stop — a hard turn boundary.** Execute exactly the one step you were told to execute — nothing from later steps, no drive-by refactors. `<plan_context>` is background, NOT a work queue: seeing "Step N+1: NOT STARTED" there does not authorize you to start it; only `<current_task>` authorizes work. When the instructed step is done (gates run, state.md updated), your turn ENDS with your report — the bridge relays it to the Lead Architect, who decides what happens next. Report real results, including failures — never gloss over red output.
- **Trust your tools' output.** Read each command's result from what was returned — never re-run a command "to confirm" it worked. Re-running doubles latency and can repeat side effects; if the output was unclear, inspect state with a cheap read-only check (`git status --short`) instead.
- **Pre-commit gate (in order, every commit):**
  ...
  3. Invoke the `Principal Engineer` subagent to review. **Brief it by size, and never paste a large diff into the brief — that overloads the reviewer's context and kills the review before it can emit a `Verdict:`.** Judge size from `git diff --stat`:
     - **Small step** (≲8 files and ≲300 changed lines): inline the diff (or the per-file diffs) directly in the brief.
     - **Large step** (>~8 files or >~300 changed lines): do NOT paste the diff. Pass the `git diff --stat` output, the step goal, the relevant `plan.md` excerpt, and a named list of the highest-risk files — and instruct the reviewer to pull per-file diffs itself (it has its own context-budget protocol).
     Always state the diff **baseline** in the brief: for the current step's uncommitted work that is `git diff` / `git diff HEAD` (NOT `HEAD~1`). **Also pass the relevant portion of the approved plan** (from `plan.md` in the external state dir) so it can reconcile the implementation against what was planned, not just the prompt you were handed. Fix blocking issues. **A review only counts if it came back from the subagent tool call with a `Verdict:` line — quote that verdict verbatim in your report. Reading the diff yourself and declaring it fine is NOT a review; self-approval is a contract violation even when you are confident.**
...
- **Reviewer failure:** If the Principal Engineer subagent returns nothing or errors, retry once — but the retry MUST be smaller, never the same brief again: send only `git diff --stat` plus the 2–3 highest-risk files' diffs (context overload is the most common cause of an empty reviewer response, so a fatter retry just dies again). If it fails a second time, committing is FORBIDDEN — report `REVIEW_UNAVAILABLE` and wait. Never self-approve.
- **Never push or open a PR** unless explicitly instructed in that turn. When you do, first have the `Principal Engineer` review the *entire* accumulated diff — but brief it by size exactly as in the pre-commit gate above: a multi-step feature diff is almost always "large", so send `git diff --stat main...HEAD` plus the goal and highest-risk files and let the reviewer pull per-file diffs itself; never paste the whole accumulated diff. Do not open the PR until it approves.
